...
def load_user_artists(user_artists_file: Path) -> scipy.sparse.csr_matrix:
    """
    load the user artists file and return a user-artists matirx in csr format

    Args:
        user_artists_file (Path): _description_

    Returns:
        scipy.sparse.csr_matrix: _description_
    """
...
class ArtistRetriever:
    """This class gets the artist name from the artist ID."""
    ...
    def load_artists(self, artists_file: Path) -> None:
        """Load the artists file and stores it as a df in a private attribute"""
